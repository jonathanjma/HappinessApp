  ...
  return (
    <>
      ...
      <div className="mr-auto flex flex-col ml-3 mt-3 drop-shadow-md ">
        ...
        <div className="h-1/2 w-full bg-white p-2 rounded-b-2xl flex flex-row" onClick={() => {
        ...
        {/* Dropdown selection */}
        <div className={dropDownShowing ? "absolute bg-white md:mt-24 mt-20 rounded-xl w-16 " : "collapse"}>
          {dateList.map((val, i, arr) => {
            if (i === props.selectedIndex) return;
            return (
              <p className=" rounded-xl p-2 text-lg md:text-2xl" onClick={() => { props.setSelectedIndex(i); setDropDownShowing(false) }}>
                {val.getDate()}
              </p>
            )
          })}
